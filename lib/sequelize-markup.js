...
function childrenToOptions(item, opts, optionalOpts) {
	...
	for (var i = 0; i < item.children.length; i++) {
		...
		} else if (child.block.type == 'ElseIf' || child.block.type == 'Else') {} else if (ignore.includes(child.block.type)) {} else throw curFile.buildCodeFrameError(child.block.path, "childToOption: bad child " + child.block.type);
	}
}
