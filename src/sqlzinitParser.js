...
import { 
	...
	childrenToOptions,
	objOrAssign,
	childrenToNamedOptions,
	generateIfChain
} from "./secondPassParser";

export function parseCallSQLZINIT(path) {
...
export function parseSQLZINIT(path) {
	...
	for (let tbl of orderedBlocks) {
		...
		if (tbl.block.type == "AssignmentExpression") {
			...
			childrenToOptions(tbl, configObj, optionalConfig, false, [], path);
	...
	if (config) {
		...
		throw path.hub.file.buildCodeFrameError(path, "no config specified");
